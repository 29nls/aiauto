...
- Provider model sekarang dapat dipilih melalui `DN_PROVIDER`: Google AI Studio Gemini, OpenAI, OpenRouter, Groq, atau endpoint OpenAI compatible custom. Endpoint tervalidasi dan key provider diperiksa secara shape guard.

- Recorder trace Minotaur opt in melalui `--farm-profile minotaur --record-trace PATH`: wrapper device merekam hanya field JSON versi 1 yang tervalidasi dengan frame ID sintetis, tanpa screenshot, respons model mentah, API key, judul window, atau teks UI bebas. Writer memakai file sementara dan `os.replace` secara atomic setelah sesi sukses atau hasil device failure yang eksplisit; kegagalan device dapat direplay sebagai `device_failure`, sedangkan kegagalan API, policy, atau safety tidak menulis trace parsial. CLI memberi peringatan, mengembalikan exit code bukan nol saat sesi recorder gagal, dan menolak path di luar profil Minotaur.
- Replay JSON versi 1 yang secret free untuk workflow Minotaur: runner memakai validasi `FarmObservationClaim`, `FarmWatchdog`, dan jalur aksi asli terhadap device in memory, tanpa OpenAI, screenshot, atau input fisik. Subcommand offline `python -m dn_bot replay <trace.json>` mencetak ringkasan singkat dan mengembalikan exit code bukan nol untuk file, schema, policy, atau expectation error. Trace memverifikasi state sebelum dan sesudah, panggilan device, kegagalan device tanpa commit state, aturan loot dan retreat, serta budget recovery.
- Profil farming Minotaur opsional (`--farm-profile minotaur --until-stopped`) dengan state machine eksplisit (`pre_dungeon` → `entering_dungeon` → `combat` → `boss_reward` → `loot_chest` → `loot_result` → `retreat_dialog` → `return_wait`), watchdog timeout/progres yang masuk recovery terbatas (dan berhenti bila recovery macet), skip box/review, klik peti loot yang terlihat, dan navigasi F12 terpisah dari dialog Stage Entrance/Town. Mode `--dry-run` tetap tersedia untuk rehearsal tanpa input fisik. Budget aksi per run dan recovery kumulatif mencegah loop recovery tanpa batas.
...
- Mode latihan **`--dry-run`** (kandidat survey #3): flag CLI baru menyuntikkan `DryRunDevice` (implementasi kedua protocol `DeviceInput` di `device.py`) ke sesi — loop penuh capture → model → aksi → frame baru berjalan persis seperti biasa, tetapi setiap primitif fisik yang dimaksud hanya **di-log** (prefix `[dry-run]`) dan direkam, tanpa mengeksekusi apa pun. `run_dn_bot` kini menerima `device=` (default adapter produksi, perilaku non-dry-run byte-identical) dan meng-thread-nya ke guard emergency + aksi; `position()` dry-run mengembalikan koordinat aman sehingga cek emergency stop asli lolos, sementara `Ctrl+C`/fokus jendela tetap menjadi mekanisme abort.

### Changed

- Klaim `farm_state` dari model kini dibungkus sebagai `FarmObservationClaim` yang immutable dan tidak tepercaya. `FarmWatchdog.state` tetap menjadi state authoritative Python; `validate_claim` hanya menghasilkan kandidat dan `advance` tetap melakukan commit setelah device berhasil. Wire API, mode default, dry run, serta policy fase Minotaur tidak berubah.
...
- Adapter OpenAI (`_call_openai`) kini mengembalikan `ModelReply` polos (teks + tool requests terurai) dan mem-parse respons SDK di luar loop retry; `api.py` menjadi satu-satunya modul yang menyentuh object SDK, `extract_tool_requests` mengembalikan `list[ToolRequest]` (kandidat arsitektur #2).
...
- Tujuan retreat Minotaur kini dapat dikunci operator dengan `DN_RETREAT_DESTINATION` atau `--retreat-destination` (`stage_entrance`/`town`), dengan precedence CLI > env > mode legacy saat unset. Watchdog tetap memvalidasi label dan koordinat screenshot, dan prompt sesi menyebut tujuan yang dipilih.
- Preflight kini menolak `OPENAI_API_KEY` yang jelas tidak valid — tanpa prefix `sk-` atau terlalu pendek (`OPENAI_KEY_MIN_LENGTH`), termasuk placeholder `.env.example` — dengan pesan actionable sebelum countdown, bukan menunggu 401 saat runtime (temuan survey T7).
...
- `check_emergency_stop` kini meneruskan `EmergencyStop`/`FocusLost` yang dilempar device seam tanpa membungkus ulang (re-raise sebelum broad `except Exception`), konsisten dengan konvensi hierarki exception / pola `_call_openai` (temuan survey baru).
- `.env.example` dibersihkan dan menambahkan `DN_INSTRUCTION` (variabel yang didukung sejak T3); `OPENAI_TIMEOUT` sudah tercantum sebelumnya.
...
Versi awal eksperimen vision input untuk Dragon Nest via OpenAI + tool calling, diimplementasikan sebagai script tunggal `app_dn.py`.

### Added

- Loop orchestrator: capture → model vision OpenAI → validasi aksi allow-listed → frame baru.
...
- Retry/backoff terbatas untuk error transien OpenAI (429, 5xx, koneksi) tanpa mengulang aksi; error konfigurasi gagal cepat dengan pesan penyebab spesifik.
- Preflight konfigurasi sebelum countdown 5 detik: platform Windows, `OPENAI_API_KEY`, `OPENAI_MODEL`, `DN_WINDOW_TITLE`, dan variabel `DN_CAPTURE_*`/`DN_MONITOR`.
